...
void shift(int stateMove) {
    vector<string> tmp;
    Node node = Node(1, input.front(), tmp, idx ++);
...
void apply(int k) {
    ...
    while (pop_sz --) {
        stateStk.pop();
        childs.push_back(nodes.back().id);
        nodes.pop_back();
    }
    reverse(childs.begin(), childs.end());
    Token tmp;
    Node root(2, tmp, rules[k], idx ++);
...
void output(Node& node) {
    if (node.type == 1) {
        cout << (node.token).kind << ' ' << (node.token).lexeme << endl;
    } else {
    ...
    for (int i = 0; i < (node.children).size(); i ++) {
        output(allNodes[(node.children)[i]]);
    }
    return;
}
...
int main() {
    ...
    while (getline(ss, line)) {
        ...
        int stateId, ruleId;
        string nextSymb;
        subss >> stateId >> ruleId >> nextSymb;
        addreduce(stateId, ruleId, nextSymb);
        subss.clear();
    }
    // read input
    Token BOFToken = {"BOF", "BOF"};
    Token EOFToken = {"EOF", "EOF"};
    input.push_back(BOFToken);
    ...
    while (input.size()) {  
        ...
        string nextInput = input.front().kind;
