...
int getNext(int cur, char transEdge) {
    if (charToPosMap.count(transEdge) == 0) {
      return 0;
    }
    int edge = charToPosMap[transEdge];
...
bool search(std::string& s) {
    int i = 0;
    int node = 1; // root, initial state
    std::string munchedInput = "";
    while (true) {
      int next_node = 0;
      if (i < s.size()) {
        next_node = getNext(node, s[i]);
      } else {
        next_node = 0;
      }
      if (next_node == 0) {
        // ERROR
        if (!nodes[node].isAccept()) {
          return false; // report error and exit
        }
        if (node != 1) {
          std::cout << munchedInput << std::endl;
          munchedInput = "";
        }
        node = 1;
        if (i == s.size()) {
